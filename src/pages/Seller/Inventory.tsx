...
export default function Inventory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      stock: Number(stock),
      description,
      image: image || 'https://images.unsplash.com/photo-1486262715619-67b85e0b08d3?auto=format&fit=crop&w=400&q=80',
    };
    ...
    <MobileContainer hasBottomNav>
      <div className="p-6 space-y-6">
        ...
        <div className="flex gap-3">
          ...
          <button
            className={`w-14 h-14 rounded-2xl bg-card border border-border shadow-xs text-muted-foreground flex items-center justify-center shrink-0 transition-colors cursor-pointer relative ${isFilterOpen || activeCategory !== 'All' ? 'border-primary/30 text-primary bg-primary/5' : 'hover:bg-muted'
              }`}
        ...
        {/* Category Tabs - Horizontally Scrollable Collapsible Layout */}
        {isFilterOpen && (
          <div className="w-full overflow-x-auto custom-scrollbar pb-2 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="flex gap-2 w-max px-1">
              {['All', ...categories].map((cat) => (
                <button
                  key={cat}
                  className={`px-4 py-2.5 rounded-full font-bold text-xs text-center transition-all cursor-pointer select-none whitespace-nowrap ${activeCategory === cat
        ...
        {/* Inventory List - 2 Column Grid */}
        <div className="pb-20">
          {filteredProducts.length === 0 ? (
            <div className="text-center p-8 text-muted-foreground text-sm bg-card rounded-3xl border border-border">
              No products found in this category. Click the "+" button to add inventory.
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-6">
              {filteredProducts.map((item) => (
                <Card key={item.id} className="rounded-[20px] border border-border bg-card p-1.5 flex flex-col justify-between hover:shadow-md hover:border-primary/20 transition-all duration-300 active:scale-[0.98]">
                  {/* Nested Image Container */}
                  <div className="h-32 w-full relative bg-muted rounded-[15px] overflow-hidden shrink-0">
                    <img
                      src={item.image || item.image_url || 'https://images.unsplash.com/photo-1486262715619-67b85e0b08d3?auto=format&fit=crop&w=400&q=80'}
                      alt={item.name}
                  ...
                  <div className="px-1 pb-0.5 flex-1 flex flex-col justify-between space-y-1">
                    <div className="space-y-1">
                      <h4 className="font-bold text-[13px] leading-snug line-clamp-2 mt-[-10px]">{item.name}</h4>
